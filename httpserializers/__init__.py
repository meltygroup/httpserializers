...
import json
from typing import Optional, Union, List, Dict, Any, Set, Sequence
...
def serialize(document: Document, accept_header: str, hostname: str) -> bytes:
    """Serialize the given document according to the Accept header of the
    given request.
    """
    return json.dumps(serializers[accept_header](document, hostname), indent=4).encode(
        "UTF-8"
    )
